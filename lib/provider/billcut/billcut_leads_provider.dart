...
class BillCutLeadsProvider extends ChangeNotifier {
  final ApiService _apiService = ApiService();
  FirebaseFirestore? _firestore;

  List<BillcutLead> _leads = [];
  ...
  List<BillcutLead> get leads => _leads;
  ...
  Future<void> fetchLeads({
    ...
    try {
      final url = Endpoints.fetchBillCutLeads(
        name,
        limit,
      ...
      print("Fetch BillCut Leads Response: $decoded");
      final List list = decoded['data'] ?? [];
      final fetched = list.map((e) => BillcutLead.fromFirestore(e)).toList();
      ...
      debugPrint("Fetch BillCut Leads Error: $e");
    } finally {
      _isLoading = false;
      _hasLoadedOnce = true;
      notifyListeners();
  ...
  Future<void> fetchMore({int limit = 20}) async {
    ...
    try {
      ...
      final List list = decoded['data'] ?? [];
      _leads.addAll(list.map((e) => BillcutLead.fromFirestore(e)).toList());

      _nextCursorId = decoded['nextCursorId'];
      _hasMore = decoded['hasMore'] ?? false;
    } catch (e) {
      debugPrint("Fetch More BillCut Leads Error: $e");
    } finally {
  ...
  Future<void> fetchAdminLeads({
    ...
    try {
      final url = Endpoints.fetchBillCutLeadsAdmin(
        limit,
        reset ? null : _nextCursorId,
        search: _search,
      );
      ...
      final decoded = jsonDecode(res.body);
      // print(res.statusCode);
      // print("Fetch Admin BillCut Leads Response: $decoded");
      final List list = decoded['data'] ?? [];
      final fetched = list.map((e) => BillcutLead.fromFirestore(e)).toList();
      ...
      debugPrint("Fetch Admin BillCut Leads Error: $e");
    } finally {
      _isLoading = false;
      _hasLoadedOnce = true;
      notifyListeners();
  ...
  Future<void> fetchMoreAdmin({int limit = 20}) async {
    ...
    try {
      ...
      debugPrint("Fetch More Admin BillCut Leads Error: $e");
    } finally {
      _isFetchingMore = false;
      notifyListeners();
    }
